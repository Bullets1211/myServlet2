...
public class ManageUsers extends HttpServlet {
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		out.println("function goPage(){"
				...
				+ "window.open('/myServlet2/ManageUsers?pagenow='+pagenow.value,'_self');}"
		...
		for (Users u : al) {
			out.println("<tr><td>"
					...
					+ u.getGrade()
					+ "</td><td>"
					+ "<a onclick='return conformdel()' href='/myServlet2/UserServlet?type=del&id="
					+ u.getId() + "'>删除用户</a>" + "</td><td>"
					+ "<a href='/myServlet2/UserServlet?type=gotoupdview&id="
					+ u.getId() + "'>修改用户</a>" + "</td></tr>");
		}
		out.println("</table>");
		if (pagenow != 1) {
			out.println("<a href='/myServlet2/ManageUsers?pagenow="
					+ (pagenow - 1) + "'><上一页></a>");
		} else {
			out.println("<a href='/myServlet2/ManageUsers?pagenow=" + (pagenow)
					+ "'><上一页></a>");
		}
		for (int i = 1; i <= pagecount; i++) {
			out.println("<a href='/myServlet2/ManageUsers?pagenow=" + i + "'><"
					+ i + "></a>");
		}
